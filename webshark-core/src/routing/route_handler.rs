...
pub trait RouteHandler<Args> {
    fn invoke(&self, req: Request<Bytes>) -> Response<Bytes>;
}

/// Реализация для функций и замыканий, не принимающих аргументов.
impl<F> RouteHandler<()> for F
where
    F: Fn() -> Response<Bytes> + Send + Sync + 'static,
{
    fn invoke(&self, _req: Request<Bytes>) -> Response<Bytes> {
        self()
    }
}

/// Реализация для функций и замыканий, принимающих [`Request`] по значению.
impl<F> RouteHandler<Request<Bytes>> for F
where
